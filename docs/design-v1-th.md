...
3. **Retire threshold 30-40% — ไม่เอาแบบ % ตายตัว** แก้เป็น 2 กลไก: (a) STATE SNAPSHOT ในทุกรายงาน (เพื่อสื่อสาร); (b) retire เมื่อ remaining < ~2× งานถัดไป หรือ staleness (ยึด assumption เก่า/อ้างไฟล์ที่ย้าย → `staleness_flags`) หมายเหตุ: budget จริง 15M/ตัว ⇒ retirement จะ **เกิดน้อยมาก** — อย่า over-build กลไกนี้ **[แก้รอบ 6 — operator เถียง: "test หมดแล้ว ไม่มีเคส agent หายจนใช้ตัวเดิมไม่ได้ การเขียน handoff เพิ่มเปลืองโดยใช่เหตุ" — ถูกตามหลักฐาน P-H/T7c]**: ตัดพิธี append handoff file ต่อ report ทิ้ง; transcript บนดิสก์คือ handoff ตัวจริง (ชุบได้ทั้ง context); เมื่อชุบไม่ได้ (คนละ lineage/transcript ถูก prune) ค่อย harvest snapshot ล่าสุดจาก transcript on-demand — จ่ายเมื่อใช้ ไม่จ่ายตลอด; handoff file เหลือเฉพาะ deliberate retirement + delegator-handoff ของตัว delegator เอง; residual: transcript retention (cleanupPeriodDays) ต้องตั้งยาวพอ campaign
