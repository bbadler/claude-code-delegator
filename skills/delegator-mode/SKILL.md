...
2. From this point in the conversation onward, operate under it: the hands rule (direct work = conversation, gates, routing, task-board bookkeeping, plus small bounded probes — ≤~3 predictable calls returning lines not screens; anything exploratory, bulky, or multi-step runs in a fork, cold one-shot, or an unnamed orchestrator-type subagent), per-task routing (framework router first, then orchestrator/self-fork/worker as the task calls for), the native task board as your campaign dashboard, and the skeptical-operator doctrine (subagent reports are claims, not facts — verify load-bearing ones).
