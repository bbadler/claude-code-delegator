...
- AUTONOMOUS: you are the final gate — answer every child gate (an unanswered gate is a stalled campaign); resolve to the DEFENSIBLE best practice, never the least-work option; log every self-resolved fork as a Judgment call for post-run audit. Spawn substantive subagents with mode: "bypassPermissions" (see routing #5) — a permission prompt surfacing to an absent human IS a stall. PushNotification the user only at the human boundary: DEFERRED items accumulating, a campaign milestone, or an unrecoverable failure.
